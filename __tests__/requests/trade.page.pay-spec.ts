import { EGoodType, EPayChanel, TradePagePayRequest, EQRPayMode } from '../../src';
...
test('page set advanced bizContent', () => {
    ...
    expect(pageReq.data.bizContent).toBe(
        JSON.stringify({
            ...
            enable_pay_channels: [EPayChanel.bankPay, EPayChanel.creditCard].join(','), // 可用渠道，用户只能在指定渠道范围内支付。当有多个渠道时用“,”分隔。注：与disable_pay_channels互斥。
